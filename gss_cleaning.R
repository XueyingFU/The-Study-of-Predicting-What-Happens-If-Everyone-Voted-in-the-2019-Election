...
raw_data <- read_csv('AAeK0FX5.csv')
...
raw_census_data <- read_csv("gss.csv")

# Add the labels
raw_census_data <- labelled::to_factor(raw_census_data)


names(raw_census_data)[names(raw_census_data) == "education"] <- "education_level"
...
cleaned_census_data <- 
  reduced_census_data %>%
  filter(region != "n/a", sex != "n/a", age != "n/a", education_level != "n/a", marital_status != "n/a") %>%
  mutate(if_married = 
           if_else(marital_status == "married, spouse present", 1,
                   if_else(marital_status == "married, spouse absent", 1, 0)))

# Post Stratification
cleaned_census_data <- 
  cleaned_census_data %>%
  count(region, sex, age, education_level,  marital_status) %>%
  group_by(region, sex, age, education_level, marital_status)
...
# working directory
write_csv(cleaned_census_data, "census_data.csv")
